  ...
  return (
    ...
    >
      ...
      <div className="mb-4 flex gap-2">
        <button
          className={`px-3 py-1.5 rounded-full border ${tab==='main' ? 'bg-emerald-600 text-white border-emerald-600' : 'border-emerald-200 text-emerald-700'}`}
          onClick={() => setTab('main')}
        >
          Profil
        </button>
        <button
          className={`px-3 py-1.5 rounded-full border ${tab==='stats' ? 'bg-emerald-600 text-white border-emerald-600' : 'border-emerald-200 text-emerald-700'}`}
      ...
      {tab === 'stats' ? (
        <div className="mb-6">
          {me?.uid ? <UserStats uid={me.uid} /> : <p className="text-emerald-700">Ładowanie…</p>}
        </div>
      ) : (
        <>
          {/* Zaproszenia */}
